  ...
  return (
    <div>
      <div className='  flex items-center gap-12 mt-20'>
        ...
        <div>
        <span className='text-[#FCEFD5] bg-[#194B52] py-1 px-2 text-[0.50rem] rounded-full mx-14'>YUSUF HANDICRAFTS</span>
        <h1 className='text-7xl font-bold mx-14 mb-20 '>HANDICRAFTED <br></br>FURNITURE FOR <br></br>EVERY CORNER </h1>
        </div>
        <div className='w-[50%] h-[70vh] mr-[10]  '>
        <Swiper
        spaceBetween={6}
        slidesPerView={1}
        breakpoints={{
    ...
    <div className='flex items-center gap-12 mx-[10%] mb-[0%]'>
      <div className=''>
...
Join us on this journey of creativity and tradition. Discover the world of Yusuf Handicrafts and experience the art of true craftsmanship.</p>
        <Link to='/rooms' className='py-[2px]  px-4 border border-zinc-900 rounded-3xl flex items-center gap-2 font-normal mt-6 w-[8vw]'>Services<IoIosArrowRoundForward className='font-bold text-3xl mt-1'/></Link>
       </div>
  ...
  <div className='w-[80vw] h-[70vh]'>
    ...
    >
      {description.map((image, index) => (
        <SwiperSlide key={index}>
          <div className="flex items-center bg-white shadow-md border border-gray-100 rounded-md w-full h-[50vh]">
            <img src={image.src} alt={image.alt} className='w-[300px] p-8 h-[360px] mx-10 object-cover rounded-md' />
            
            <div className="mt-2 mx-4 p-4 w-[48vw]">
              <h3 className="text-lg font-semibold mb-4">{image.title}</h3>
              <p className="text-gray-700">{image.description}</p>
              <Link to='/products' className='py-[2px] px-4 border border-zinc-900 rounded-3xl flex items-center gap-4 font-normal mt-6 w-1/4'>
                Explore More<IoIosArrowRoundForward className='font-bold text-3xl mt-1'/>
              </Link>
            </div>
          </div>
        </SwiperSlide>
